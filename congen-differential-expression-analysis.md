...
R  
EdgeR (installation described below)  
...
```{r}
setwd("/Users/jlkelley/OneDriveWSU/OneDrive - Washington State University (email.wsu.edu)/ConGen2020/analyses")
```

### Read in gene count matrix 
```{r}
counts = read.csv("ballgown/gene_count_matrix.csv", header = T, row = 1)
```
...
### perform quasi-likelihood F-test for differential expression between the two groups
```{r}
